...
public class GeoHelper {
		...
		public static Dataset fromGeoJSON(OpenWareDataItem data)throws IllegalArgumentException {
			...
			for(long ts: geojsons.keySet()) {
				...
				for(JSONObject obj: geojsons.get(ts)) {
					...
					case "point":res.addAll(extractPointsFromPoint(ts, obj));break;
					case "multipoint":res.addAll(extractPointsFromLineOrMultiPoint(ts, obj));break;
					case "linestring":res.addAll(extractPointsFromLineOrMultiPoint(ts, obj));break;
					case "multilinestring":res.addAll(extractPointsFromPolygonOrMultiLine(ts, obj));break;
					case "polygon":res.addAll(extractPointsFromPolygonOrMultiLine(ts, obj));break;
					case "multipolygon":res.addAll(extractPointsFromMultipolygon(ts, obj));break;
					case "geometrycollection":res.addAll(extractPointsFromGeometryCollection(ts, obj,null));break;
					default:
		...
		private static List<Instance> extractPointsFromGeometryCollection(long ts, JSONObject collection, List<Instance> instances){
			List<Instance> data;
			if(instances == null) {
				data = new ArrayList<>();	
			}else {
			...
			for(int i=0; i<geometries.length(); i++) {
				...
				case "point":data.addAll(extractPointsFromPoint(ts, geometries.getJSONObject(i)));break;
				case "multipoint":data.addAll(extractPointsFromLineOrMultiPoint(ts, geometries.getJSONObject(i)));break;
				case "linestring":data.addAll(extractPointsFromLineOrMultiPoint(ts, geometries.getJSONObject(i)));break;
				case "multilinestring":data.addAll(extractPointsFromPolygonOrMultiLine(ts, geometries.getJSONObject(i)));break;
				case "polygon":data.addAll(extractPointsFromPolygonOrMultiLine(ts, geometries.getJSONObject(i)));break;
				case "multipolygon":data.addAll(extractPointsFromMultipolygon(ts, geometries.getJSONObject(i)));break;
				case "geometrycollection":data.addAll(extractPointsFromGeometryCollection(ts, geometries.getJSONObject(i),data));break;
				default:
